...
async function fetchGames(game, numberGame) {
...
async function showGames(game, range) {
...
// Sem uso no momento...
async function showGameInfo(game, numberGame) {
    const response = await fetch(`https://api.guidi.dev.br/loteria/${game}/${numberGame}`);
    const gameResult = await response.json();
...
async function main(game, range) {
...
async function euroMilhao(range) {
